...
namespace NSBase
{
	namespace NSFunction
	{
		...
		{
#if SHOW_MEM_COPY == 1
			CNSCallCounter allocCounter( 2, SHOW_TICK, "fbbase memory copy" );
#endif
			...
			if ( size <= 128 )
			{
				return memcpy_tiny( destination, source, size );
			}
		...
		void* memmove_fast( void* des, const void* src, size_t size )
		{
...
#if SHOW_MEM_MOVE == 1
			CNSCallCounter allocCounter( 3, SHOW_TICK, "fbbase memory move" );
#endif
			...
			if ( s < d && d <= s + size )
				...
				return memcpy_fast( des, src, size );
		}

		CNSString getStackInfo( )
		{
			// 使用static变量，减少内存分配次数
			static CNSString stackInfo;
			stackInfo = sStackInfo;
			sStackInfo.clear( );
			return stackInfo;
		}
#ifdef PLATFORM_IOS
		...
		void stackTrace( CONTEXT* context )
		{
			...
			SymInitialize( curProcess, NULL, TRUE );
			...
			while ( StackWalk64( IMAGE_FILE_MACHINE_AMD64, curProcess, curThread, &sf, context, NULL, SymFunctionTableAccess, SymGetModuleBase, NULL ) )
			{
				BOOL success = TRUE;
				if ( SymFromAddr( curProcess, sf.AddrPC.Offset, &dwDisplament, symInfo ) == FALSE )
					success = FALSE;
		...
		CDir gRoot;
		CNSMap< CNSString, CDir* >		gDirIndex;
		CNSMap< CNSString, CNSOctets >	gFileData;

		void initFAT( )
		{
			HRSRC tFindRes = FindResourceA( NULL, "file", MAKEINTRESOURCEA( 10 ) );
			if ( tFindRes == NULL )
				return;

			HANDLE tFindHandle = LoadResource( NULL, tFindRes );
			if ( tFindHandle == NULL )
				return;

			DWORD tSize = SizeofResource( NULL, tFindRes );
			void* tpBuffer = LockResource( tFindHandle );
			CNSOctets tRawBuffer( tpBuffer, tSize );

			CNSOctets unCompressBuffer = tRawBuffer.uncompress( );

			CNSVector< CFATable > tFATs;
			CNSVector< CNSOctets > tDatas;
			CNSOctetsStream tStream( unCompressBuffer );
			tStream >> tFATs;
			tStream >> tDatas;

			for ( unsigned int i = 0; i < tFATs.getCount( ); i ++ )
			{
				CNSString tFileName = tFATs[ i ].mFileName;
				static CNSVector< CNSString > tNameList;
				tNameList.clear( );
				tFileName.split( "/", tNameList );
				CNSString tNameIndex;
				CDir* tpParent = &gRoot;
				for ( unsigned int t = 0; t < tNameList.getCount( ); t ++ )
				{
					tNameIndex = ( t == 0 ? tNameList[ t ] : tNameIndex + "/" + tNameList[ t ] );
					if ( t < tNameList.getCount( ) - 1 )
					{
						CDir** tpDirRef = gDirIndex.get( tNameIndex );
						CDir* tpDir = NULL;
						if ( tpDirRef == NULL )
						{
							tpDir = new CDir;
							gDirIndex.insert( tNameIndex, tpDir );
							tpDir->mName = tNameList[ t ];
							tpParent->mDirs.pushback( tpDir );
							tpParent = tpDir;
						}
						else
						{
					...
					{
						tpParent->mFiles.pushback( tNameList[ t ] );
						gFileData.insert( tNameIndex, tDatas[ tFATs[ i ].mAddress ] );
					}
				}
			}
		}
		...
		{
			CDir** tpDirRef = gDirIndex.get( rPath );
			if ( ( *tpDirRef ) == NULL )
				return NULL;
		...
		{
			CNSOctets* tpData = gFileData.get( rPath );
			if ( tpData == NULL )
				return false;
