...
static long long unicode_transliterate (conv_t cd, ucs4_t wc,
                                  unsigned char* outptr, size_t outleft)
{
  if (cd->oflags & HAVE_HANGUL_JAMO) {
    ...
    ucs4_t buf[3];
	long long ret = johab_hangul_decompose(cd,buf,wc);
    if (ret != RET_ILUNI) {
      /* we know 1 <= ret <= 3 */
  ...
  }
  {
    ...
    if (indx >= 0) {
      ...
      size_t backup_outleft = outleft;
      unsigned int i;
	  unsigned long long sub_outcount;
      for (i = 0; i < num; i++) {
        if (outleft == 0) {
          sub_outcount = RET_TOOSMALL;
                                        ...
                                        void* callback_arg)
{
  ...
  if (plocals->l_errno == 0) {
    ...
    outcount_ok:
      ...
      #endif
      if (!(outcount <= (long long) outleft)) abort();
                                    ...
                                    char* * outbuf, size_t *outbytesleft)
{
  ...
  outcount_zero:
    if (!(incount <= (long long) inleft)) abort();
    inptr += incount; inleft -= incount;
  }
  ...
  if (outbuf == NULL || *outbuf == NULL) {
    ...
    if (cd->ifuncs.xxx_flushwc) {
      ...
      if (cd->ifuncs.xxx_flushwc(cd, &wc)) {
        unsigned char* outptr = (unsigned char*) *outbuf;
        size_t outleft = *outbytesleft;
		long long outcount = cd->ofuncs.xxx_wctomb(cd,outptr,wc,outleft);
        if (outcount != RET_ILUNI)
          goto outcount_ok;
        /* Handle Unicode tag characters (range U+E0000..U+E007F). */
        if ((wc >> 7) == (0xe0000 >> 7))
