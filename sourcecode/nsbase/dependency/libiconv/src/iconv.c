...
#define stringpool2 ((char *) &stringpool2_contents)
static const struct alias sysdep_aliases[] = {
#define S(tag,name,encoding_index) { (long long)&((struct stringpool2_t *)0)->stringpool_##tag, encoding_index },
#include "aliases2.h"
#undef S
};
#ifdef __GNUC__
...
struct nalias { char* name; unsigned int encoding_index; };
...
static int compare_by_name (void const* arg1, void const * arg2)
{
...
#define aliascount  (aliascount1+aliascount2)
    ...
    qsort(aliasbuf, num_aliases, sizeof(struct nalias), compare_by_index);
  {
    ...
    while (j < num_aliases) {
      ...
      if (i > 1)
		  qsort( namesbuf, i, sizeof( char const* ), compare_by_name);
      /* Call the callback. */
